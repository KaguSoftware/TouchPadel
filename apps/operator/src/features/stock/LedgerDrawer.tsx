  ...
  const columns: Column<MovementRow>[] = [
    {
      ...
      render: (m) => <bdi>{formatDateTime(new Date(m.at), locale)}</bdi>,
    },
    {
      key: 'type',
      header: tr('op.stock.movement'),
      render: (m) => <code style={{ fontSize: 'var(--tp-fs-xs)' }}>{m.movement_type}</code>,
    },
    {
      key: 'qty',
      ...
      render: (m) => (
        <span
          style={{
            color: m.qty_delta < 0 ? 'var(--tp-danger-fg)' : 'var(--tp-success-fg)',
            fontWeight: 600,
          }}
          dir="ltr"
        >
    ...
    },
    {
      ...
      numeric: true,
      render: (m) => <Money amount={m.unit_cost_iqd} />,
    },
    {
      key: 'ref',
      header: tr('op.stock.ref'),
      render: (m) => (
    ...
    {
      key: 'reason',
      header: tr('op.common.reason'),
      render: (m) => <span style={{ color: 'var(--tp-muted-fg)' }}>{m.reason_code ?? '—'}</span>,
    },
  ];
