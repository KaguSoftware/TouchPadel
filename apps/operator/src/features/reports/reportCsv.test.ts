...
describe('reportCsv', () => {
  const cols = normalizeColumns(
    ['court', 'revenue_iqd', 'occupancy_pct'],
    [{ court: 'A', revenue_iqd: 1, occupancy_pct: 2 }],
  );
  ...
  it('writes localised headers, raw numbers and a totals row', () => {
    const csv = reportCsv(
      cols,
      [{ court: 'A', revenue_iqd: 15000, occupancy_pct: 42.5 }],
      { revenue_iqd: 15000, occupancy_pct: 42.5 },
      label,
      'Total',
    );
  ...
  it('omits the totals row when there are no totals', () => {
    const csv = reportCsv(
      cols,
      [{ court: 'A', revenue_iqd: 1, occupancy_pct: null }],
      null,
      label,
      'Total',
    );
...
describe('reportFilename', () => {
  ...
  it('names the report, the range and every active filter', () => {
    expect(
      reportFilename('courts', period, {
        view: 'byHour',
        court: '1a2b3c4d-0000-4000-8000-000000000000',
        pay: undefined,
      }),
    ).toBe('courts_2026-09-01_2026-09-30_view-byHour_court-1a2b3c4d.csv');
  });
  it('strips characters that do not belong in a filename', () => {
    expect(reportFilename('cafe', period, { cat: 'hot drinks/tea' })).toBe(
      'cafe_2026-09-01_2026-09-30_cat-hotdrinkstea.csv',
    );
  });
});
