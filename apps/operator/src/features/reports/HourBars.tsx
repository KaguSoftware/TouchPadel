...
export function HourBars({
  ...
  return (
    ...
    >
      ...
      <h2 style={{ fontSize: 'var(--tp-fs-sm)', color: 'var(--tp-muted-fg)', fontWeight: 600 }}>
        {title}
      </h2>
      {rows.map((row, i) => {
        const v = values[i] ?? 0;
        ...
        const label = labelCol
          ? formatCell(row[labelKey], labelCol.kind, locale, units)
          : String(row[labelKey] ?? '');
        return (
          <div
            key={i}
          ...
          >
            <span dir="ltr" style={{ fontVariantNumeric: 'tabular-nums', textAlign: 'start' }}>
              {label}
            </span>
            <span
              style={{
