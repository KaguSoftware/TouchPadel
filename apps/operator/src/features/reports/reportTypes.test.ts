...
describe('normalizeColumns', () => {
  it('accepts bare keys and specs, carrying both labels', () => {
    const cols = normalizeColumns(
      ['court', { key: 'revenue_iqd', label_en: 'Rev', label_ar: 'إيراد' }],
      [{ court: 'A', revenue_iqd: 10 }],
    );
    expect(cols).toEqual([
  ...
  it('falls back to the first row, hiding ids and putting the time first', () => {
    const cols = normalizeColumns(null, [
      { amount_iqd: 5, court_id: 'x', at: '2026-01-01T00:00:00Z', kind: 'sale', id: 'y' },
    ]);
