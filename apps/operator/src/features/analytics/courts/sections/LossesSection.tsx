...
export function LossesSection({ raw, state, refreshing, f, rangeLabel }: SectionProps) {
  ...
  const byLead = e ? join(e.cancellations.byLeadTime, e.noShows.byLeadTime, leadKeys, (k) => segmentLabel(tr, f, 'byLeadTime', k)) : [];
  ...
  const noticeRows = (e?.cancellations.byNotice ?? []).map((b) => ({ label: segmentLabel(tr, f, 'byNotice', b.bucket), value: b.n }));
  ...
  return (
    <>
      <ZoneGrid columns={2}>
        ...
        {stacked(tr('ws.analytics.courts.cards.lossByLead'), tr('ws.analytics.courts.tips.losses'), byLead, 'losses-by-lead', 200, 10)}
      </ZoneGrid>
      <div style={{ marginBlockStart: 'var(--tp-sp-3)' }}>
        <ZoneGrid columns={3}>
          <ChartCard
            ...
            note={g(e?.cancellations)?.medianNoticeMin != null ? `${tr('ws.analytics.courts.cards.medianNotice')}: ${f.duration((e?.cancellations.medianNoticeMin ?? 0) * 60)}` : undefined}
          ...
          >
            <CountBars rows={noticeRows} format={(n) => f.num(n)} name={tr('ws.analytics.courts.units.cancellations')} tickFontSize={10} />
          </ChartCard>
          <StatPair
