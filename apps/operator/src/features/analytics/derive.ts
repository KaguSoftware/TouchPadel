...
export function derive(raw: RawAnalytics): Derived {
  ...
  const datesWithSales = raw.daily
    .filter((d) => d.revenueIqd > 0 || d.orders > 0)
    .map((d) => d.date);
  ...
  const salesDeltaReliable =
    coverage.ratio >= RELIABLE_COVERAGE && coveragePrev.ratio >= RELIABLE_COVERAGE;
  ...
  const perPersonIqd =
    coversEstimated && coversEstimated > 0
      ? Math.round(salesWithin(raw.daily, engNow) / coversEstimated)
      : null;
  ...
  const viewsById = hasEng
    ? ph.topViewed.filter((v) => keep(v.id)).map((v) => ({ id: v.id, count: v.sessions }))
    : [];
  const cartsById = hasEng
    ? ph.topCarted.filter((v) => keep(v.id)).map((v) => ({ id: v.id, count: v.sessions }))
    : [];
  const itemConversion = hasEng
    ? buildItemConversion(viewsById, cartsById, soldTotals, names, 500)
    : [];
  const gems = hasEng ? hiddenGems(itemConversion) : [];
  const momentum = itemMomentum(
    viewsById,
    prev
      ? prev.topViewed.filter((v) => keep(v.id)).map((v) => ({ id: v.id, count: v.sessions }))
      : [],
    engNow,
    engPrev,
    names,
  ...
  const costed = raw.menu
    ...
    .map((m) => ({
      ...
      nameEn: m.nameEn,
      nameAr: m.nameAr,
      defaultPriceIqd: m.priceIqd,
      costIqd: m.costIqd,
    }));
  const menuEngineering = buildMenuEngineering(soldTotals, costed, {
    popularityRule: 0.7,
    reliableCoverage: 0.6,
  });
  ...
  for (const d of raw.daily)
    engByDate.set(d.date, { date: d.date, views: 0, waiterCalls: d.waiterCalls });
  if (hasEng) {
  ...
  for (const m of raw.menu)
    if (m.categoryId)
      categoryNames.set(m.categoryId, { nameEn: m.categoryNameEn, nameAr: m.categoryNameAr });
