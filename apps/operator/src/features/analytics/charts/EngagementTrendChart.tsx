...
export function EngagementTrendChart({ rows, f }: { rows: readonly SalesVsEngagementDay[]; f: Formatters }) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} syncId={TREND_SYNC_ID} margin={{ top: 4, right: 8, bottom: 4, left: 8 }}>
        ...
        <Line type="monotone" dataKey="views" name={tr('analytics.cards.viewsSeries')} stroke={SERIES_2} dot={false} strokeWidth={2} />
        <Line type="monotone" dataKey="waiterCalls" name={tr('analytics.cards.callsSeries')} stroke={SERIES_3} dot={false} strokeWidth={1.5} />
      </LineChart>
    </ResponsiveContainer>
  );
}
