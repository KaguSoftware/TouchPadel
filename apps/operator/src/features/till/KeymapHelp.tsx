      ...
      >
        {TILL_KEYMAP.map((row) => (
          <div key={row.labelKey} style={{ display: 'contents' }}>
            <dt
              style={{ display: 'inline-flex', gap: 'var(--tp-sp-0)', flexWrap: 'wrap' }}
              dir="ltr"
            >
              {row.keys.map((k, i) => (
                <Kbd key={i}>{k}</Kbd>
            ...
            <dd style={{ margin: 0 }}>
              {tr(`ws.cashier.till.help.${row.labelKey}` as MessageKey)}
            </dd>
          </div>
        ))}
      </dl>
