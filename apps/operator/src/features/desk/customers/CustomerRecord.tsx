...
const FLAG_TYPES: readonly CustomerFlagType[] = [
  'vip',
  'birthday',
  'payment_note',
  'special_request',
...
export function CustomerRecordScreen() {
  ...
  const courtName = (cid: string) =>
    pickName(
      locale,
      courtsQ.data?.find((c) => c.id === cid),
    ) || cid;
  ...
  const status =
    recordQ.isError && !recordQ.data
      ? 'error'
      : recordQ.data === undefined
        ? 'loading'
        : recordQ.data === null
          ? 'empty'
          : 'ready';
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          rec ? (
            ...
            >
              {rec.flags.length === 0 ? (
                <span>{tr('ws.courtDesk.record.noFlags')}</span>
              ) : (
                rec.flags.map((f, i) => (
      ...
      <AsyncStateWrapper
        status={status}
        error={recordQ.error}
        onRetry={() => void recordQ.refetch()}
        emptyContent={<EmptyState icon="users" title={tr('ws.courtDesk.record.notFound')} />}
      >
        {rec && (
          <div
            style={{
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              <Panel>
                <DescriptionList
                  ...
                  items={[
                    {
                      label: tr('ws.courtDesk.record.phone'),
                      value: rec.customer.phone ? <bdi dir="ltr">{rec.customer.phone}</bdi> : '—',
                    },
                    {
                      label: tr('ws.courtDesk.record.email'),
                      value: rec.customer.email ? <bdi dir="ltr">{rec.customer.email}</bdi> : '—',
                    },
                    {
                      label: tr('ws.courtDesk.record.language'),
                      value:
                        rec.customer.preferred_lang === 'ar'
                          ? tr('ws.courtDesk.customers.lang.ar')
                          : rec.customer.preferred_lang === 'en'
                            ? tr('ws.courtDesk.customers.lang.en')
                            : '—',
                    },
                    {
                      label: tr('ws.courtDesk.record.bookings'),
                      value: formatNumber(counts.bookings, locale),
                      numeric: true,
                    },
                    {
                      label: tr('ws.courtDesk.record.cancellations'),
                      value: formatNumber(counts.cancellations, locale),
                      numeric: true,
                    },
                    {
                      label: tr('ws.courtDesk.record.noShows'),
                      value: formatNumber(counts.noShows, locale),
                      numeric: true,
                    },
                    {
                      label: tr('ws.courtDesk.record.cafeOrders'),
                      value: formatNumber(counts.cafeOrders ?? rec.cafeOrders.length, locale),
                      numeric: true,
                    },
                    ...(rec.customer.created_at
                      ? [
                          {
                            label: tr('ws.courtDesk.record.since'),
                            value: (
                              <bdi>{formatDate(new Date(rec.customer.created_at), locale, tz)}</bdi>
                            ),
                          },
                        ]
                      : []),
              ...
              <BookingsPanel
                title={tr('ws.courtDesk.record.upcoming')}
                empty={tr('ws.courtDesk.record.upcomingEmpty')}
                rows={rec.upcoming}
                tz={tz}
                courtName={courtName}
              />
              <BookingsPanel
                title={tr('ws.courtDesk.record.history')}
                empty={tr('ws.courtDesk.record.historyEmpty')}
                rows={rec.history}
                tz={tz}
                courtName={courtName}
              />
              <Panel title={tr('ws.courtDesk.record.cafe')} padded={rec.cafeOrders.length === 0}>
                {rec.cafeOrders.length === 0 ? (
                  <p style={{ color: 'var(--tp-muted-fg)' }}>
                    {tr('ws.courtDesk.record.cafeEmpty')}
                  </p>
                ) : (
              ...
              <Panel title={tr('ws.courtDesk.record.series')}>
                {rec.series.length === 0 ? (
                  <p style={{ color: 'var(--tp-muted-fg)' }}>
                    {tr('ws.courtDesk.record.seriesEmpty')}
                  </p>
                ) : (
                  <ul
                    style={{
                    ...
                    {rec.series.map((s) => (
                      ...
                      >
                        ...
                        <span style={{ color: 'var(--tp-muted-fg)', fontSize: 'var(--tp-fs-sm)' }}>
                          <bdi>
                            {formatDate(new Date(`${s.starts_on}T12:00:00Z`), locale, 'UTC')} –{' '}
                            {formatDate(new Date(`${s.ends_on}T12:00:00Z`), locale, 'UTC')}
                          </bdi>
                          {s.occurrences !== undefined && (
                            <>
                              {' '}
                              ·{' '}
                              {tr('ws.courtDesk.record.occurrences', {
                                count: formatNumber(s.occurrences, locale),
                              })}
                            </>
                          )}
                        </span>
...
function BookingsPanel({
  ...
  title: string;
  empty: string;
  rows: readonly CustomerReservationRow[];
  tz: string;
  courtName: (id: string) => string;
}) {
  const { locale } = useLocale();
  // The RPC carries the court's names with each row; the courts query is only the fallback.
  const nameOf = (r: CustomerReservationRow) =>
    r.court_name_en && r.court_name_ar
      ? pickName(locale, { name_en: r.court_name_en, name_ar: r.court_name_ar })
      : courtName(r.court_id);
  const navigate = useNavigate();
  return (
  ...
  return (
    <Panel title={tr('ws.courtDesk.record.notes')} data-testid="customer-notes">
      <MessagePresenter
        tone="info"
        icon="lock"
        message={tr('ws.courtDesk.record.notesLead')}
        style={{ marginBlockEnd: '0.75rem' }}
      />
      {notes.length === 0 ? (
        <p style={{ color: 'var(--tp-muted-fg)', marginBlockEnd: '0.75rem' }}>
          {tr('ws.courtDesk.record.noNotes')}
        </p>
      ) : (
        <ul
          style={{
      ...
      <Field label={tr('ws.courtDesk.record.addNote')}>
        <textarea
          ...
          disabled={busy}
          maxLength={2000}
          placeholder={tr('ws.courtDesk.record.notePlaceholder')}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <Button
          ...
          busy={busy}
          disabled={!draft.trim()}
          disabledReason={tr('ws.courtDesk.record.noteEmpty')}
          onClick={() => void add()}
        >
  ...
  return (
    ...
    >
      ...
      >
        ...
        {note.edited_at && (
          <span title={formatDateTime(new Date(note.edited_at), locale, tz)}>
            ...
            {note.edited_by_name
              ? ` (${tr('ws.courtDesk.record.by', { name: note.edited_by_name })})`
              : ''}
          </span>
        )}
        {!editing && (
      ...
      {editing ? (
        <div>
          <textarea
            style={{ ...inputStyle, minBlockSize: '4rem', resize: 'vertical' }}
            value={body}
            disabled={busy}
            maxLength={2000}
...
function FlagsEditor({
  ...
  async function save() {
    ...
    setError(null);
    try {
      const p_flags = FLAG_TYPES.filter((t) => state[t].on).map((t) => ({
        type: t,
        label: state[t].label.trim() || null,
      }));
          ...
          >
            ...
            >
              <input
                ...
                checked={state[t].on}
                disabled={busy}
                onChange={(e) => setState({ ...state, [t]: { ...state[t], on: e.target.checked } })}
              />
