...
type Pending =
  { kind: 'occurrence'; occurrence: SeriesOccurrence } | { kind: 'series'; scope: Scope };
...
export function SeriesDetailScreen() {
    ...
    try {
      if (pending.kind === 'occurrence') {
        await mutate('reservation.update', {
          action: 'cancel',
          reservationId: pending.occurrence.id,
          reason,
        });
      } else {
        await appRpc('cancel_series', {
          p_series_id: id,
  ...
  const status =
    seriesQ.isError && !seriesQ.data
      ? 'error'
      : seriesQ.data === undefined
        ? 'loading'
        : seriesQ.data === null
          ? 'empty'
          : 'ready';
  ...
  const patternText = s
    ? s.pattern === 'weekdays'
      ...
      : tr(
          `ws.courtDesk.seriesDetail.patternLabel.${s.pattern === 'fortnightly' ? 'fortnightly' : 'weekly'}`,
        )
    : '';
  ...
  return (
    <div>
      <PageHeader
        ...
        title={
          s
            ? (s.guest_name ?? tr('ws.courtDesk.common.walkIn'))
            : tr('ws.courtDesk.seriesDetail.title')
        }
          ...
          <>
            ...
            {s && !cancelled && (
              <Button
                ...
                disabledReason={
                  summary.upcoming === 0
                    ? tr('ws.courtDesk.seriesDetail.nothingToCancel')
                    : undefined
                }
      ...
      <AsyncStateWrapper
        ...
        emptyContent={<EmptyState icon="repeat" title={tr('ws.courtDesk.seriesDetail.notFound')} />}
      >
        {s && (
          <div style={{ display: 'grid', gap: '1rem' }}>
            {cancelled && (
              <MessagePresenter
                tone="info"
                message={tr('ws.courtDesk.seriesDetail.seriesCancelled')}
              />
            )}
            <Panel>
              <DescriptionList
                ...
                items={[
                  ...
                  {
                    label: tr('ws.courtDesk.seriesDetail.court'),
                    value: <bdi>{courtName(s.court_id)}</bdi>,
                  },
                  {
                  ...
                  {
                    label: tr('ws.courtDesk.series.time'),
                    value: <bdi dir="ltr">{s.start_time.slice(0, 5)}</bdi>,
                  },
                  {
                    label: tr('ws.courtDesk.series.duration'),
                    value: tr('op.common.minutesShort', { minutes: s.duration_min }),
                  },
                ]}
              />
              {s.notes && (
                <p style={{ marginBlockStart: '0.75rem', color: 'var(--tp-muted-fg)' }}>
                  {s.notes}
                </p>
              )}
            </Panel>
            ...
            >
              ...
              <table
                className="tp-table"
                data-dense="true"
                aria-label={tr('ws.courtDesk.seriesDetail.occurrences')}
              >
                <thead>
                  <tr>
                ...
                <tbody>
                  {occurrences.map((o) => {
                    ...
                    return (
                      <tr
                        key={o.id}
                        aria-disabled={o.played || undefined}
                        style={{ opacity: o.played ? 'var(--tp-opacity-disabled)' : 1 }}
                      >
                        ...
                        <td style={{ whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>
                          <bdi>
                            {formatTimeRange(new Date(o.start_at), new Date(o.end_at), locale, tz)}
                          </bdi>
                        </td>
                        <td>
                          ...
                          >
                            ...
                            {o.played && (
                              <StatusBadge
                                ...
                                tone="neutral"
                                icon="check"
                                label={tr('ws.courtDesk.seriesDetail.played')}
                                title={tr('ws.courtDesk.seriesDetail.untouchable')}
                              />
                            )}
                          </span>
                        </td>
                        <td style={{ textAlign: 'end', whiteSpace: 'nowrap' }}>
                          {o.played ? (
                            ...
                            >
                              {tr('ws.courtDesk.seriesDetail.untouchable')}
                            </span>
                          ) : (
                            <span style={{ display: 'inline-flex', gap: '0.3rem' }}>
                              <Button
                                ...
                                iconEnd="chevronEnd"
                                onClick={() =>
                                  void navigate({ to: '/desk/bookings/$id', params: { id: o.id } })
                                }
                              >
                              ...
                              {editable && (
                                <Button
                                  ...
                                  kind="danger"
                                  disabled={busy}
                                  onClick={() => setPending({ kind: 'occurrence', occurrence: o })}
                                >
      ...
      {scopeDialog && (
        <Modal
          ...
          footer={
            <>
              <Button onClick={() => setScopeDialog(false)}>
                {tr('ws.courtDesk.seriesDetail.keep')}
              </Button>
              <Button
                kind="danger"
        ...
        >
          <div
            role="radiogroup"
            aria-label={tr('ws.courtDesk.seriesDetail.scopeChoice')}
          ...
          >
            {(['future', 'all'] as const).map((sc) => (
              <label
                ...
                className="tp-row"
                data-clickable="true"
                data-selected={scope === sc ? 'true' : undefined}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '0.5rem',
              ...
              >
                ...
                <span style={{ flex: 1 }}>
                  {sc === 'future'
                    ? tr('ws.courtDesk.seriesDetail.scopeFuture')
                    : tr('ws.courtDesk.seriesDetail.scopeAll')}
                </span>
                <span style={{ color: 'var(--tp-muted-fg)', fontVariantNumeric: 'tabular-nums' }}>
                  {tr('ws.courtDesk.record.occurrences', {
                    count: formatNumber(cancelScopeCount(occurrences, sc, nowIso), locale),
                  })}
                </span>
              </label>
            ))}
      ...
      {pending && (
        <ReasonCodePrompt
          action={
            pending.kind === 'series'
              ? tr('ws.courtDesk.seriesDetail.reasonCancelSeries', {
                  count: formatNumber(cancelScopeCount(occurrences, pending.scope, nowIso), locale),
                })
