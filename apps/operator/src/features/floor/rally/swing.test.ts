// Copied from apps/mobile/src/features/courtTransition/__tests__/swing.test.ts
// (2026-09-18); two reach figures retuned for the floor's RACKET_SCALE.
...
describe('the rig (mount → pivot → lay → hold → racket)', () => {
  ...
  it('the top view lays the head into the court plane; the front view stands it up', () => {
    ...
    expect(up.y).toBeGreaterThan(0.4); // head well above the hand (FLOOR: 0.7 at the phone's 4.2 scale)
  });
  ...
  it('the stroke throws the head a metre and a half off its resting spot', () => {
    ...
    for (const lay of [0, -Math.PI / 2]) {
      ...
      // FLOOR: 1.5 m at the phone's 4.2 scale; the reach scales with the arm.
