// Copied from apps/mobile/src/features/courtTransition/__tests__/racket.test.ts
// (2026-09-18); the size case is the floor's.
...
describe('the racket mesh (padel-racket.html)', () => {
  ...
  it('is a racket a drawn player can hold, taller than it is wide', () => {
    const kit = buildRacketKit('full');
    const rig = kit.create(1);
    rig.lay.rotation.x = 0;
    const size = boxOf(rig).getSize(new THREE.Vector3());
    // FLOOR: RACKET_SCALE 2.5 — about a metre long beside a 1.06 m capsule.
    expect(Math.max(size.x, size.y)).toBeGreaterThan(0.9);
    expect(Math.max(size.x, size.y)).toBeLessThan(1.4);
    expect(size.z).toBeLessThan(0.3); // a flat racket, not a paddle-shaped blob
  });
