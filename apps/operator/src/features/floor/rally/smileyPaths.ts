/*
 * COPIED from apps/mobile/src/features/courtTransition/smileyPaths.ts on 2026-09-18 for the
 * operator's live floor plan (owner request: "the padel rackets, the ones in the
 * mobile app, with their animation"). The mobile file is the source of truth for
 * the artwork and the stroke; the two apps pin different three.js versions
 * (mobile 0.160, operator 0.184) under a hoisted node_modules, which is why this
 * is a copy and not a shared package. Differences from the source are marked
 * `FLOOR:`; keep everything else byte-identical so a diff shows only them.
 */
/**
 * THE SMILEY BALL — the brand's artwork, as frozen path data and nothing else.
 *
 ...
 * and painted underneath, so the sticker reads on any surface. Neither a racket
 * face nor a loading screen is a sticker, so only the twelve coloured paths are
 * here.
 *
 * FOUR COLOURS IN THE PDF, THREE ON THE MARK. The extraction paints navy
 ...
 * two greens are therefore one group here, and each renderer maps the three
 * that remain onto colours it already wears rather than adding a hue: see the
 * decal in `racket.ts` for which, and for why the navy is NOT the plate's own
 * blue (the PDF's navy against the plate is (53, 78, 168) against (51, 96, 171)
 * — the mark would have dissolved into the face).
 *
 * THE GROUPS ARE PAINT LAYERS, and drawing them out of order takes the mark
 * apart: INK, then BALL and SEAM, then INK_TOP. `smileyMark.ts` documents why
 * three layers are enough; every renderer owes them that order.
 *
 ...
 * the right reader for this artwork as it was for the wordmark — and what lets
 * the SVG renderer take the browser's default nonzero fill and land the same
 * mark.
 */
