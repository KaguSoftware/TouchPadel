...
export function ManagementPanelScreen() {
  ...
  const money = (n: number) =>
    Number.isInteger(n) ? formatIQD(n, locale) : formatNumber(n, locale);
  ...
  function exportCsv() {
    const csv = toCsv(
      [
        tr('ws.owner.panel.csv.figure'),
        tr('ws.owner.panel.csv.value'),
        tr('ws.owner.panel.csv.previous'),
        tr('ws.owner.panel.csv.changeAbs'),
        tr('ws.owner.panel.csv.changePct'),
      ],
  ...
  return (
    <div>
      ...
      <Toolbar
          ...
          <ComparisonControl mode={compare} onChange={setCompare} disabled={headlineQ.isFetching} />
        }
      >
      ...
      <AsyncStateWrapper
        ...
        emptyContent={
          <EmptyState
            ...
            action={
              <Button onClick={() => setPeriod(presetPeriod('last30'))}>
                {tr('ws.kit.dateRange.last30')}
              </Button>
            }
          />
        }
      >
        <section
          aria-label={tr('ws.owner.panel.headline')}
          style={{
            display: 'grid',
          ...
          <Panel
            ...
            actions={
              <Button
                ...
                onClick={() => go('/reports/courts')}
              >
                {tr('ws.owner.panel.openCourts')}
              </Button>
            }
          >
            <FigureRows
              metas={figuresIn('padel')}
              figures={figures}
              compare={compare}
              label={label}
              valueOf={valueOf}
          ...
          <Panel
            ...
            actions={
              <Button
                ...
                kind="ghost"
                iconEnd="arrowUpRight"
                onClick={() => go('/reports/cafe')}
              >
                {tr('ws.owner.panel.openCafe')}
              </Button>
            }
          >
            <FigureRows
              metas={figuresIn('cafe')}
              figures={figures}
              compare={compare}
              label={label}
              valueOf={valueOf}
        ...
        <nav
          aria-label={tr('ws.shell.nav.reports')}
          style={{
            display: 'flex',
            gap: 'var(--tp-sp-2)',
            flexWrap: 'wrap',
        ...
        >
          <Button size="sm" icon="chart" onClick={() => go('/reports/revenue')}>
            {tr('ws.owner.panel.openRevenue')}
          </Button>
          <Button size="sm" icon="box" onClick={() => go('/reports/stock')}>
            {tr('ws.owner.panel.openStock')}
          </Button>
          <Button size="sm" icon="users" onClick={() => go('/reports/staff')}>
            {tr('ws.owner.panel.openStaff')}
          </Button>
        </nav>
      </AsyncStateWrapper>
        ...
        const body = (
          <>
            ...
            <Icon
              name="arrowUpRight"
              size={14}
              style={{ color: 'var(--tp-muted-fg)', visibility: drillable ? 'visible' : 'hidden' }}
            />
          </>
        );
        return (
