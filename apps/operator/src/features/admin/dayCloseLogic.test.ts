...
describe('deriveDayCloseState', () => {
  ...
  it('blocks on open tabs before anything else the manager could fix later', () => {
    ...
    expect(
      deriveDayCloseState({ ...base, openTabCount: 2, error: new Error('DAY_OPEN_TABS') }),
    ).toBe('blockedByOpenTabs');
    expect(deriveDayCloseState({ ...base, openTabCount: 1, queuedCount: 3 })).toBe(
      'blockedByOpenTabs',
    );
  });
  ...
  it('closed wins over everything, including a stale error', () => {
    expect(
      deriveDayCloseState({ ...base, closed: true, error: new Error('x'), openTabCount: 1 }),
    ).toBe('closed');
  });
});
...
describe('dayCloseCsv', () => {
  const labels: CsvLabels = {
    ...
    waste: 'Waste',
    openingFloat: 'Float',
    cashPayments: 'Cash in',
    cardPayments: 'Card in',
  };
  const summary = {
    day_session_id: 'd1',
    ...
    card_payments_iqd: 80000,
    cash_expected_iqd: 170000,
    cash_counted_iqd: 168000,
    cash_variance_iqd: -2000,
    card_expected_iqd: 80000,
    card_terminal_batch_iqd: 80000,
    discounts_iqd: 15000,
    adjustment_count: 2,
    authorizer_names: ['Dev Manager', 'Dev Owner'],
    voided_lines_iqd: 4000,
    voided_line_count: 1,
    refunds_iqd: 0,
    refund_count: 0,
    waste_cost_iqd: 2500,
      ...
      [
        {
          ...
          reason_code: 'comp',
          created_at: '',
          applied_by_name: 'Sara',
          authorized_by_name: 'Dev Manager',
        },
      ],
      (names) => names.join(', '),
    );
