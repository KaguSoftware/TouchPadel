...
export function StaffAccountEditor({
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (nameDirty)
        await appRpc('rename_staff', { p_staff_id: staff.id, p_display_name: name.trim() });
  ...
  return (
    <Panel
      title={
        <span style={{ display: 'inline-flex', gap: 'var(--tp-sp-2)', alignItems: 'center' }}>
          ...
          {dirty && (
            <StatusBadge tone="warn" size="sm" label={tr('ws.owner.staff.editor.unsaved')} />
          )}
        </span>
      }
      actions={
        <Button
          ...
          size="sm"
          icon="x"
          aria-label={tr('ws.owner.staff.editor.close')}
          onClick={onClose}
          disabled={busy}
        />
      }
    ...
    >
      {isSelf && (
        <MessagePresenter
          tone="info"
          message={tr('ws.owner.staff.editor.selfNote')}
      ...
      <div style={{ marginBlockEnd: 'var(--tp-sp-3)' }}>
        ...
        {holdsPin ? (
          ...
          >
            <StatusBadge
              tone={staff.has_pin ? 'success' : 'neutral'}
              size="sm"
              label={staff.has_pin ? tr('op.staff.pinSet') : tr('op.staff.pinNone')}
            />
          ...
          <span style={{ fontSize: 'var(--tp-fs-sm)', color: 'var(--tp-muted-fg)' }}>
            {tr('ws.owner.staff.editor.pinNotApplicable')}
          </span>
        )}
        <span
          style={{
        ...
        >
          {tr('ws.owner.staff.editor.pinNote')}
        </span>
      </div>
      ...
      <div style={{ marginBlockEnd: 'var(--tp-sp-3)' }}>
        <Switch
          checked={active}
          label={tr('ws.owner.staff.editor.enabled')}
          disabled={locked || busy}
        ...
        >
          {tr('ws.owner.staff.editor.enabledNote')}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--tp-sp-2)', justifyContent: 'flex-end' }}>
        ...
        <Button
          ...
          icon="check"
          busy={save.isPending}
          disabled={!dirty || !canManage || clearPin.isPending}
          onClick={() => void submit()}
        >
