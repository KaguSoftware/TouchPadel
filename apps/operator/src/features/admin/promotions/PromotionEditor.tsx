...
export function PromotionEditorScreen() {
  ...
  return (
    <AsyncStateWrapper
      ...
      emptyContent={
        <MessagePresenter tone="error" message={tr('ws.manager.promotions.editor.notFound')} />
      }
    >
...
function Editor({ id, row }: { id: string | null; row: PromotionRow | null }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const result = await appRpc<string | { id: string } | null>(
        'upsert_promotion',
        toRpcArgs(draft, id),
      );
      const newId =
        typeof result === 'string' ? result : result && typeof result === 'object' ? result.id : id;
      return newId ?? id;
    },
    onSuccess: async (newId) => {
      ...
      if (id === null && newId)
        void navigate({ to: '/admin/promotions/$id', params: { id: newId }, replace: true });
    },
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {lc && !dirty && (
              <StatusBadge
                tone={lc === 'live' ? 'success' : lc === 'scheduled' ? 'info' : 'neutral'}
                label={tr(`ws.manager.promotions.${lc}`)}
              />
            )}
            <Button onClick={() => void navigate({ to: '/admin/promotions' })}>
              {tr('ws.kit.actions.back')}
            </Button>
            <Button
              kind="ghost"
            ...
            <Button
              ...
              icon="check"
              busy={save.isPending}
              disabled={readOnly || !dirty || errors.length > 0}
              onClick={() => save.mutate()}
            >
      ...
      >
        ...
        <Panel title={tr('ws.manager.promotions.editor.basics')}>
          <BilingualFieldPair
            ...
            disabled={readOnly}
            error={
              errors.includes('name') ? tr('ws.manager.promotions.editor.errors.name') : undefined
            }
          />
          <Field label={tr('ws.manager.promotions.editor.type')}>
            <SegmentedControl
              ...
              onChange={(type) =>
                patch({
                  type,
                  value: type === 'percent' ? Math.min(99, Math.max(1, draft.value)) : draft.value,
                })
              }
              options={[
          ...
          {draft.type === 'percent' ? (
            <Field
              ...
              error={
                errors.includes('percent') || errors.includes('value')
                  ? tr('ws.manager.promotions.editor.errors.percent')
                  : undefined
              }
            >
              <PercentInput
            ...
            <Field
              label={tr('ws.manager.promotions.editor.amountValue')}
              error={
                errors.includes('value')
                  ? tr('ws.manager.promotions.editor.errors.value')
                  : undefined
              }
            >
              <MoneyInput
                value={draft.value}
                onChange={(v) => patch({ value: v ?? 0 })}
                disabled={readOnly}
              />
            </Field>
          )}
        ...
        <Panel title={tr('ws.manager.promotions.editor.whenTitle')}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--tp-sp-2-5)' }}>
            ...
            <Field
              ...
              error={
                errors.includes('dates')
                  ? tr('ws.manager.promotions.editor.errors.dates')
                  : undefined
              }
            >
              <input
          ...
          <Field
            label={tr('ws.manager.promotions.editor.weekdays')}
            hint={tr('ws.manager.promotions.editor.weekdaysHint')}
          >
          ...
          <Field
            ...
            error={
              errors.includes('hours') ? tr('ws.manager.promotions.editor.errors.hours') : undefined
            }
          >
        ...
        <Panel title={tr('ws.manager.promotions.editor.scopeTitle')}>
          ...
          >
            {tr('ws.manager.promotions.editor.scopeHint')}
          </p>
          <ChipPicker
            ...
            disabled={readOnly}
            onToggle={(cid) =>
              patch({ scope: { ...draft.scope, courtIds: toggleId(draft.scope.courtIds, cid) } })
            }
          />
          <ChipPicker
            label={tr('ws.manager.promotions.editor.categories')}
            options={(menu.data?.categories ?? []).map((c) => ({
              id: c.id,
            ...
            onToggle={(cid) =>
              patch({
                scope: { ...draft.scope, categoryIds: toggleId(draft.scope.categoryIds, cid) },
              })
            }
          />
          <ChipPicker
            label={tr('ws.manager.promotions.editor.items')}
            options={(menu.data?.items ?? [])
              .filter((i) => i.is_active)
              .map((i) => ({ id: i.id, label: pickName(locale, i) }))}
            selected={draft.scope.itemIds}
            disabled={readOnly}
            searchable
            onToggle={(iid) =>
              patch({ scope: { ...draft.scope, itemIds: toggleId(draft.scope.itemIds, iid) } })
            }
          />
        </Panel>
        ...
        <Panel title={tr('ws.manager.promotions.editor.limitsTitle')}>
          ...
          >
            {tr('ws.manager.promotions.editor.limitHint')}
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--tp-sp-2-5)' }}>
            <Field label={tr('ws.manager.promotions.editor.limitTotal')}>
              <CountInput
                value={draft.limits.total}
                disabled={readOnly}
                onChange={(total) => patch({ limits: { ...draft.limits, total } })}
              />
            </Field>
            <Field label={tr('ws.manager.promotions.editor.limitPerCustomer')}>
              <CountInput
                ...
                disabled={readOnly}
                onChange={(perCustomer) => patch({ limits: { ...draft.limits, perCustomer } })}
              />
            </Field>
          </div>
          <Field label={tr('ws.manager.promotions.editor.minSpend')}>
            <MoneyInput
              value={draft.limits.minSpendIqd}
              allowEmpty
              disabled={readOnly}
              onChange={(minSpendIqd) => patch({ limits: { ...draft.limits, minSpendIqd } })}
            />
          </Field>
        </Panel>
          ...
          >
            <div>
              <Switch
                ...
                label={tr('ws.manager.promotions.editor.autoLabel')}
              />
              <p
                style={{
              ...
              >
                {tr('ws.manager.promotions.editor.autoHint')}
              </p>
            </div>
            <div>
              <span
              ...
              >
                {tr('ws.manager.promotions.editor.publicCode')}
              </span>
              <div
                style={{
                  display: 'flex',
              ...
              >
                ...
                <Button
                  ...
                  title={
                    id === null || dirty
                      ? tr('ws.manager.promotions.editor.generateHint')
                      : undefined
                  }
            ...
            <div>
              <Switch
                ...
                label={tr('ws.manager.promotions.editor.enabledLabel')}
              />
              <p
                style={{
                  fontSize: 'var(--tp-fs-xs)',
...
function ChipPicker({
  ...
  const shown = q.trim()
    ? options.filter(
        (o) => o.label.toLowerCase().includes(q.trim().toLowerCase()) || selected.includes(o.id),
      )
    : options;
  return (
    <fieldset style={{ border: 'none', padding: 0, margin: 0, marginBlockEnd: 'var(--tp-sp-3)' }}>
      ...
      >
        {label}
        {selected.length > 0 && (
          <span style={{ color: 'var(--tp-muted-fg)', fontWeight: 400 }}> · {selected.length}</span>
        )}
      </legend>
      {searchable && (
        <input
