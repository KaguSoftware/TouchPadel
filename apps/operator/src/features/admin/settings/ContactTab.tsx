  ...
  return (
    <div style={{ maxInlineSize: 'var(--tp-measure-form)' }}>
      ...
      >
        ...
        {venueQ.data && (
          <Panel
            title={tr('ws.owner.settings.tabs.contact')}
            actions={<StatusBadge tone="neutral" size="sm" label={tr('ws.kit.common.readOnly')} />}
          >
            <DescriptionList
              items={[
                {
                  label: tr('ws.owner.settings.contact.venueName'),
                  value: <bdi>{venueQ.data.venue_name}</bdi>,
                },
                {
                  label: tr('ws.owner.settings.contact.phone'),
                  value: venueQ.data.phone ? (
                    <span dir="ltr" style={{ fontVariantNumeric: 'tabular-nums' }}>
                      {venueQ.data.phone}
                    </span>
                  ) : (
                    <span style={{ color: 'var(--tp-muted-fg)' }}>
                      {tr('ws.owner.settings.contact.phoneNone')}
                    </span>
                  ),
                },
                {
                  label: tr('ws.owner.settings.contact.timezone'),
                  value: <span dir="ltr">{venueQ.data.timezone}</span>,
                },
              ]}
            />
            <MessagePresenter
              tone="info"
              message={tr('ws.owner.settings.contact.readOnlyNote')}
