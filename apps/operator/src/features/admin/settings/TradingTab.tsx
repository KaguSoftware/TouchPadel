...
export function TradingTab() {
  ...
  const venueQ = useQuery({
    queryKey: VENUE_ADMIN_KEY,
    queryFn: fetchVenueAdmin,
    staleTime: 60_000,
  });
  ...
  const taxColumns: Column<TaxGroupRow>[] = [
    {
      key: 'name',
      header: tr('ws.owner.settings.trading.taxGroup'),
      render: (g) => <bdi>{pickName(locale, g)}</bdi>,
    },
    {
      key: 'rate',
      header: tr('ws.owner.settings.trading.taxRate'),
      numeric: true,
      render: (g) => `${formatNumber(bpToPercent(g.rate_bp), locale)}%`,
    },
    {
      key: 'status',
      ...
      render: (g) =>
        g.is_active ? (
          <StatusBadge tone="success" size="sm" label={tr('ws.kit.common.on')} />
        ) : (
          <StatusBadge
            tone="neutral"
            size="sm"
            label={tr('ws.owner.settings.trading.taxInactive')}
          />
        ),
    },
  ];
  ...
  return (
    ...
    >
      <AsyncStateWrapper
        status={asyncStatus(venueQ, () => false)}
        error={venueQ.error}
        onRetry={() => void venueQ.refetch()}
        skeleton={<Skeleton lines={6} />}
      >
        {venueQ.data && (
          <>
            <Panel
              title={tr('ws.owner.settings.trading.currency')}
              actions={
                <StatusBadge tone="neutral" size="sm" label={tr('ws.kit.common.readOnly')} />
              }
            >
              ...
              <p style={{ color: 'var(--tp-muted-fg)', fontSize: 'var(--tp-fs-sm)' }}>
                {tr('ws.owner.settings.trading.currencyHint')}
              </p>
              <DescriptionList
                style={{ marginBlockStart: 'var(--tp-sp-2-5)' }}
                items={[
                  {
                    label: tr('ws.owner.settings.trading.taxInclusive'),
                    value: venueQ.data.tax_inclusive
                      ? tr('ws.kit.common.on')
                      : tr('ws.kit.common.off'),
                  },
                ]}
              />
            </Panel>
            ...
            <Panel
              title={tr('ws.owner.settings.trading.policyTitle')}
              actions={
                <StatusBadge tone="neutral" size="sm" label={tr('ws.kit.common.readOnly')} />
              }
            >
                ...
                items={[
                  {
                    ...
                    value: (
                      <Hinted
                        value={tr('ws.owner.settings.trading.hours', {
                          count: formatNumber(venueQ.data.cancellation_window_hours, locale),
                        })}
                  ...
                  },
                  {
                    ...
                    value: (
                      <Hinted
                        value={tr('ws.owner.settings.trading.seconds', {
                          count: formatNumber(venueQ.data.hold_ttl_seconds, locale),
                        })}
                  ...
                  },
                  {
                    ...
                    value: (
                      <Hinted
                        value={tr('ws.owner.settings.trading.hours', {
                          count: formatNumber(venueQ.data.protected_horizon_hours, locale),
                        })}
                  ...
                  },
                  {
                    ...
                    value: (
                      <Hinted
                        value={tr('ws.owner.settings.trading.days', {
                          count: formatNumber(venueQ.data.max_booking_horizon_days, locale),
                        })}
                  ...
                  },
                  {
                    ...
                    value: (
                      <Hinted
                        value={formatNumber(venueQ.data.max_live_holds_per_guest, locale)}
                        hint={tr('ws.owner.settings.trading.maxHoldsHint')}
                      />
                    ),
                  },
                  {
                    label: tr('ws.owner.settings.trading.noShow'),
                    value: (
                      <span style={{ fontSize: 'var(--tp-fs-sm)' }}>
                        {tr('ws.owner.settings.trading.noShowBody')}
                      </span>
                    ),
                  },
                ]}
              ...
              <MessagePresenter
                tone="info"
                message={tr('ws.owner.settings.trading.readOnlyNote')}
      ...
      <Panel
        title={tr('ws.owner.settings.trading.taxTitle')}
        padded={false}
        actions={<StatusBadge tone="neutral" size="sm" label={tr('ws.kit.common.readOnly')} />}
      >
        <div
          style={{
        ...
        >
          <AsyncStateWrapper
            ...
            emptyContent={
              <EmptyState compact icon="tag" title={tr('ws.owner.settings.trading.taxNone')} />
            }
          >
            <DataTable
              columns={taxColumns}
              rows={taxQ.data ?? []}
              rowKey={(g) => g.id}
              aria-label={tr('ws.owner.settings.trading.taxTitle')}
            />
          </AsyncStateWrapper>
...
function Hinted({ value, hint }: { value: string; hint: string }) {
  return (
    <span style={{ display: 'grid', gap: 'var(--tp-sp-0)' }}>
      <span
        dir="ltr"
        style={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums', textAlign: 'start' }}
      >
        {value}
      </span>
