...
describe('isTransportError — what "no connection" is allowed to mean', () => {
  ...
  it('recognises the wrappers postgrest-js / gotrue-js put around a thrown fetch', () => {
    ...
    expect(isTransportError({ name: 'AuthRetryableFetchError', message: 'x', status: 0 })).toBe(
      true,
    );
  });
  ...
  it('does NOT label server-side failures as connectivity', () => {
    ...
    expect(
      isTransportError({ message: 'permission denied for table profiles', code: '42501' }),
    ).toBe(false);
    expect(
      isTransportError({
        message: 'Could not find the function app.hold_slot in the schema cache',
      }),
    ).toBe(false);
    ...
    expect(
      isTransportError({
        message: 'JSON object requested, multiple (or no) rows returned; fetch again',
      }),
    ).toBe(false);
  ...
  it('feeds mapErrorToKey: RPC codes first, transport second, generic last', () => {
    ...
    expect(mapErrorToKey({ message: 'permission denied for table reservations' })).toBe(
      'errors.generic',
    );
