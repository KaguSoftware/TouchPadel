...
.tp-basket-line__remove { background: none; border: none; color: var(--tp-muted-fg); font-size: var(--tp-fs-sm); text-decoration: underline; padding: 0; }
.tp-basket-line__aside { display: grid; justify-items: end; gap: var(--tp-space-2); }
...
.tp-basket-empty { text-align: center; padding-block: var(--tp-space-6); display: grid; gap: var(--tp-space-4); justify-items: center; color: var(--tp-muted-fg); }
.tp-basket-empty h3 { font-family: var(--tp-font-display); font-weight: var(--tp-fw-display); text-transform: uppercase;
  letter-spacing: var(--tp-tracking-caps); font-size: var(--tp-fs-lg); color: var(--tp-fg); }
