...
1. **Manager-PIN brute force via device rotation** — `app.verify_manager_pin` (0004:100-131) rate-limits per _client-supplied_ `p_device_id`. Any authenticated cashier calls `apply_discount` with a fresh random device id per guess: 5 free guesses per string, unlimited strings → a 4-digit PIN falls in ~2000 calls, yielding manager authority over discounts, voids, price overrides, expired write-offs. Fix: key attempts on `auth.uid()` (or uid+device).
