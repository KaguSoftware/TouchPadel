...
_verified_ matching email, i.e. control of that mailbox — which already grants password reset. The
...
id-token flow writes different metadata — Google `full_name` _and_ `name` (+ `picture`), Apple **no
