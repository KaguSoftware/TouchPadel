...
Date: 2026-09-20. Status: v1 BUILT on 2026-09-20 (migrations 0108–0113, three edge functions, the map and coverage scripts, the operator drawer and page); the binding inter-lane shapes and the DECIDE defaults taken are in `build-contracts-2026-09-20.md`. Deferred from v1: analytics components and pinning (§2.8, §4.4, §5.4), the re-check button (DECIDE 10), the `posthog` tool, the eval set, Playwright. Every fact about the repo below was read from the working tree at commit `9886105` (routes, migrations 0001–0107, edge functions, i18n, the spend cap). Vendor facts about the Claude API come from the bundled API reference; anything not verified against a live endpoint is marked **UNVERIFIED**. Owner choices are marked **DECIDE** and collected at the end. Nothing here is assumed.
