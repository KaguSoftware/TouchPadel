...
| 2 | No `sql_read` in v1. |
| 3 | Embedding seam with providers `voyage` (voyage-multilingual-2, 1024 dims), `openai` (text-embedding-3-small, requested at 1024 dims via `dimensions`), `local` (Supabase `gte-small`, 384 dims zero-padded to 1024; cosine is unchanged by zero padding), `none` (full-text only). `EMBEDDING_PROVIDER` unset ⇒ `none`. |
| 4 | Chat on Claude (`ANTHROPIC_MODEL`, default `claude-opus-5`), adaptive thinking, effort `medium`, streaming, 1 h cache TTL on the frozen prefix, server-side `fallbacks: "default"`. The insights card stays on Groq for now. |
| 5 | Owner-only. |
...
| 10 | Re-check button on an answer: deferred to v1.1 (noted in the UI as absent, not disabled). |
| 11 | Terse answers by default; "explain" is a follow-up. |
| 12–13 | Analytics components and pinning (§2.8, §4.4, §5.4): **deferred**. Nothing in this build blocks them; the component cache tables are not created. |
