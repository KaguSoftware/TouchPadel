...
single one of them. What you lose is listed per service under _"What breaks without it"_.
...
| 11  | **SMS / WhatsApp OTP vendor API key** + the Send-SMS hook secret (dormant phone sign-in, vendor addition scaffold 2026-09-05 — NOT activated) | The vendor the owner contracts (OTPIQ or Twilio; owner decision D4a) + Supabase dashboard → Auth → Hooks | Blocks phone sign-in only; nothing today depends on it                     | Supabase secrets `SMS_PROVIDER`, `OTPIQ_API_KEY` / `TWILIO_*`, `SEND_SMS_HOOK_SECRET` — full order of operations in `docs/client/phone-otp-activation.md`                          |
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TELEGRAM_BOT_TOKEN`      | [@BotFather](https://t.me/BotFather) → `/newbot` → name it _Touch Cafe Orders_ → copy the `123456789:AA…` token                                                                               |
...
(_Admin → Telegram_), so the venue can change groups without a redeploy.
...
**Verify:** Operator → Admin → Telegram → _Send test message_. The `🔔 رسالة تجريبية` message must
...
| **Personal API key** (`phx_…`) | Lets our dashboard _read_ the data back | Supabase secret `POSTHOG_PERSONAL_API_KEY` |
...
the owner sets it in _Operator → Admin → Settings_ (`analytics_engagement_floor`), or hard-clip it
...
Only the _engagement_ half goes quiet (menu views, dwell time, look-but-didn't-buy, funnels), and
...
> (_Settings → Environments → staging_). That project is the client's live database. Without
