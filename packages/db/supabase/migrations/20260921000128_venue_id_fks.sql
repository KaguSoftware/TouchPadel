...
do $venue_fks_0128$
begin
  if not exists (select 1 from pg_constraint where conname = 'courts_venue_id_fkey') then
    alter table courts
      add constraint courts_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'rate_rules_venue_id_fkey') then
    alter table rate_rules
      add constraint rate_rules_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'cafe_tables_venue_id_fkey') then
    alter table cafe_tables
      add constraint cafe_tables_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'day_sessions_venue_id_fkey') then
    alter table day_sessions
      add constraint day_sessions_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'menu_categories_venue_id_fkey') then
    alter table menu_categories
      add constraint menu_categories_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'menu_items_venue_id_fkey') then
    alter table menu_items
      add constraint menu_items_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'modifier_groups_venue_id_fkey') then
    alter table modifier_groups
      add constraint modifier_groups_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'ingredients_venue_id_fkey') then
    alter table ingredients
      add constraint ingredients_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'deliveries_venue_id_fkey') then
    alter table deliveries
      add constraint deliveries_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'stock_counts_venue_id_fkey') then
    alter table stock_counts
      add constraint stock_counts_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'tax_groups_venue_id_fkey') then
    alter table tax_groups
      add constraint tax_groups_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'marketing_audiences_venue_id_fkey') then
    alter table marketing_audiences
      add constraint marketing_audiences_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'marketing_campaigns_venue_id_fkey') then
    alter table marketing_campaigns
      add constraint marketing_campaigns_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'analytics_insights_venue_id_fkey') then
    alter table analytics_insights
      add constraint analytics_insights_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'analytics_patterns_venue_id_fkey') then
    alter table analytics_patterns
      add constraint analytics_patterns_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'analytics_insight_rejections_venue_id_fkey') then
    alter table analytics_insight_rejections
      add constraint analytics_insight_rejections_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'telegram_outbox_venue_id_fkey') then
    alter table telegram_outbox
      add constraint telegram_outbox_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'telegram_actions_venue_id_fkey') then
    alter table telegram_actions
      add constraint telegram_actions_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'manager_alerts_venue_id_fkey') then
    alter table manager_alerts
      add constraint manager_alerts_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'degraded_periods_venue_id_fkey') then
    alter table degraded_periods
      add constraint degraded_periods_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'audit_log_venue_id_fkey') then
    alter table audit_log
      add constraint audit_log_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'reservations_venue_id_fkey') then
    alter table reservations
      add constraint reservations_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'reservation_series_venue_id_fkey') then
    alter table reservation_series
      add constraint reservation_series_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'guest_sessions_venue_id_fkey') then
    alter table guest_sessions
      add constraint guest_sessions_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'tabs_venue_id_fkey') then
    alter table tabs
      add constraint tabs_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'orders_venue_id_fkey') then
    alter table orders
      add constraint orders_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'tickets_venue_id_fkey') then
    alter table tickets
      add constraint tickets_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'payments_venue_id_fkey') then
    alter table payments
      add constraint payments_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'refunds_venue_id_fkey') then
    alter table refunds
      add constraint refunds_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'stock_batches_venue_id_fkey') then
    alter table stock_batches
      add constraint stock_batches_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'stock_movements_venue_id_fkey') then
    alter table stock_movements
      add constraint stock_movements_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'waiter_calls_venue_id_fkey') then
    alter table waiter_calls
      add constraint waiter_calls_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'staff_breaks_venue_id_fkey') then
    alter table staff_breaks
      add constraint staff_breaks_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'station_staff_venue_id_fkey') then
    alter table station_staff
      add constraint station_staff_venue_id_fkey
      foreign key (venue_id) references venues(id) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'device_heartbeats_venue_id_fkey') then
