-- 0140_assistant_model_choice — the owner chooses the chat model per chat, with
...
comment on column venue_settings.llm_default_model is
  '0140. The chat model a new assistant conversation uses unless it names one. Must be a key of llm_pricing. Set by app.assistant_set_default_model.';

alter table assistant_conversations
  add column if not exists model text;

comment on column assistant_conversations.model is
  '0140. This chat''s model, or NULL to follow venue_settings.llm_default_model. Must be a key of llm_pricing. Set by app.assistant_set_model; the edge function resolves the effective model per answer and stamps it on every assistant_calls row.';
...
returns boolean
language sql stable security definer set search_path = public as $assistant_model_allowed_0140$
  select p_model is not null
     and exists (select 1 from venue_settings vs where vs.llm_pricing ? p_model)
$assistant_model_allowed_0140$;

comment on function app.assistant_model_allowed(text) is
  '0140. True when venue_settings.llm_pricing has rates for the model — the only test the assistant applies to a model name.';
...
returns jsonb
language plpgsql stable security definer set search_path = public as $assistant_models_0140$
declare
  v_out jsonb;
begin
...
end $assistant_models_0140$;

comment on function app.assistant_models() is
  '0140. Owner-only: the venue default chat model and every model the pricing table can bill — the choices the per-chat switch and the settings control offer.';
...
language plpgsql security definer set search_path = public as $assistant_set_model_0140$
declare
...
begin
  ...
  return v_row;
end $assistant_set_model_0140$;

comment on function app.assistant_set_model(uuid, text) is
  '0140. Owner-only: set or clear (NULL) the model one of the caller''s chats answers with. Refuses a model llm_pricing cannot bill.';
...
returns void
language plpgsql security definer set search_path = public as $assistant_set_default_model_0140$
begin
  if not app.is_staff('owner') then
    raise exception 'FORBIDDEN' using errcode = 'P0001';
  end if;
...
end $assistant_set_default_model_0140$;

comment on function app.assistant_set_default_model(text) is
  '0140. Owner-only: the model new chats use. Refuses a model llm_pricing cannot bill. The singleton row is addressed by its non-null id, as 0104 does, so safeupdate is satisfied.';
