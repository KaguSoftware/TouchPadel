-- ===========================================================================
-- 0115 — owner assistant: analytics components and their cache.
--
...
comment on table assistant_components is
  '0115. An analytics card the assistant pipeline fills (plan §5.4): a fixed question, the catalog tools that answer it and the JSON schema the model must return. builtin rows are seeded by migration; pinned rows come from app.assistant_pin_component. Archived rows stay for their cache history.';
...
comment on table assistant_component_cache is
  '0115. One generated answer of a component for one parameter set and one state of its inputs (plan §2.8). Served by app.analytics_component while superseded_at is null and expires_at has not passed. Written only by the assistant-component edge function through app.assistant_component_upsert.';
...
returns text
language sql immutable parallel safe as $assistant_params_hash_0115$
...
$assistant_params_hash_0115$;

comment on function app.assistant_params_hash(jsonb) is
  '0115. md5 of the canonical parameter object (jsonb text, nulls stripped). Definer-only helper; a client never computes a params hash.';
...
returns jsonb
language plpgsql stable security definer set search_path = public as $assistant_component_lookup_0115$
declare
  v_hash text;
...
end $assistant_component_lookup_0115$;

comment on function app.assistant_component_lookup(text, jsonb) is
  '0115. Definer-only: the cache read behind app.analytics_component and the edge function''s pre-warm (which has no owner session). Never calls a model.';
...
returns jsonb
language plpgsql stable security definer set search_path = public as $analytics_component_0115$
begin
  if not app.is_staff('owner') then
    raise exception 'FORBIDDEN' using errcode = 'P0001';
  end if;
  return app.assistant_component_lookup(p_key, p_params);
end $analytics_component_0115$;

comment on function app.analytics_component(text, jsonb) is
  '0115. Owner-only, STABLE, never calls a model. {hit:true, content, sources, gate, generated_at, tokens, inputs_fingerprint, params_hash} when a live cache row exists for the component and these parameters, else {hit:false, params_hash, last:{…}|null} with the most recent superseded answer for the card to grey out. params_hash is computed here from the canonical parameters.';
...
returns jsonb
language plpgsql security definer set search_path = public as $assistant_component_upsert_0115$
declare
  v_key  text := p ->> 'key';
  v_hash text;
...
end $assistant_component_upsert_0115$;

comment on function app.assistant_component_upsert(jsonb) is
  '0115. Service role only (the assistant-component edge function). Stores one generation {key, params, inputs_fingerprint, content, sources, gate, tokens, expires_at?}, computing params_hash here, and supersedes every other row for the same component and parameters. Returns the row.';
...
returns int
language plpgsql security definer set search_path = public as $assistant_component_supersede_0115$
declare
  v_n int;
begin
  ...
  return v_n;
end $assistant_component_supersede_0115$;

comment on function app.assistant_component_supersede(text, text) is
  '0115. Service role only. Marks every live cache row of a component for one parameter set superseded (the edge function saw the numbers move and could not regenerate, or a rejection forced a rewrite). Returns how many rows it stamped.';
...
returns jsonb
language plpgsql stable security definer set search_path = public as $assistant_run_tool_prewarm_0115$
declare
  v_owner uuid;
begin
...
end $assistant_run_tool_prewarm_0115$;

comment on function app.assistant_run_tool_prewarm(text, jsonb) is
  '0115. Service role only: the nightly pre-warm has no owner session, so this stamps the venue owner''s uid on the transaction and calls app.assistant_run_tool unchanged (read-only wall, same catalog, same guards). Reads only; every dispatched RPC still sees an owner.';
...
language plpgsql security definer set search_path = public as $assistant_pin_component_0115$
declare
...
begin
  ...
  return v_row;
end $assistant_pin_component_0115$;

comment on function app.assistant_pin_component(text, text, text[], jsonb, jsonb) is
  '0115. Owner-only. "Pin to Analytics": stores (or replaces) a pinned component from a chat answer — its question, the tools that answered it and a schema inferred from its figures. Re-pinning an archived key revives it. A built-in key cannot be taken. Audits nothing.';
...
returns assistant_components
language plpgsql security definer set search_path = public as $assistant_archive_component_0115$
declare
  v_row assistant_components%rowtype;
begin
  if not app.is_staff('owner') then
  ...
  returning * into v_row;
  return v_row;
end $assistant_archive_component_0115$;

comment on function app.assistant_archive_component(text) is
  '0115. Owner-only. Stamps archived_at on a pinned component (idempotent); the page hides it and its cache rows stay. Built-ins refuse with COMPONENT_BUILTIN.';
...
returns void
language plpgsql security definer set search_path = public as $assistant_prewarm_nudge_0115$
declare
  v_base text;
  v_key  text;
  v_tz   text;
...
end $assistant_prewarm_nudge_0115$;

comment on function app.assistant_prewarm_nudge() is
  '0115. Hourly at :30; posts {prewarm:true} to the assistant-component edge function only when the venue''s local hour is 3 (DECIDE 12: the three default ranges are pre-warmed nightly, never generated on page open). Silent without pg_net or the functions_base_url / service_role_key secrets; swallows its own errors.';

revoke all on function app.assistant_prewarm_nudge() from public, anon, authenticated;

do $assistant_cron_0115$
begin
  begin
