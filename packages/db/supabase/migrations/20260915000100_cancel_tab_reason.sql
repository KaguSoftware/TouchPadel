-- 0100_cancel_tab_reason — app.cancel_tab records WHY the tab went.
--
...
returns jsonb
language plpgsql security definer set search_path = public as $cancel_0100$
declare
  v_day    uuid;
  v_tab    tabs%rowtype;
  v_before jsonb;
...
comment on function app.cancel_tab(uuid, text) is
  '0100 (0085). Voids an OPEN tab that has no orders, no payments, no adjustments and no booking — the tab opened on the wrong table. p_reason_code is the operator''s chosen code, optionally ''code: note'', and is mandatory (REASON_REQUIRED); it is written to the audit row. Raises TAB_NOT_EMPTY (detail names which) when there is anything to reconcile; merged and already-settled tabs raise TAB_MERGED / TAB_NOT_OPEN. Audited as tab.cancel.';
