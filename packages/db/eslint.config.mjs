...
export default [
  ...base, // typescript-eslint recommended + the repo's RTL logical-property guard
  {
    ...
    ignores: ['dist/**', 'node_modules/**', 'eslint.config.mjs', 'vitest.config.ts', '**/*.gen.ts', 'supabase/functions/**', 'supabase/.temp/**' /* the CLI's scratch from `functions serve`, gitignored */],
  },
];
