...
describe.skipIf(!up)('0068 reports and overviews', () => {
  ...
  const businessDate = async (at: Date) => {
    const { data, error } = await svc
      .schema('app')
      .rpc('business_date', { p_at: at.toISOString() });
  ...
  beforeAll(async () => {
    ...
    const { data: r } = await svc
      .from('reservations')
      .select('price_iqd, status')
      .eq('id', reservationId)
      .single();
    reservationPrice = Number((r as { price_iqd: number }).price_iqd);
    if ((r as { status: string }).status !== 'confirmed')
      throw new Error('seed booking is not confirmed');
    ...
    const { data: t } = await svc
      .from('tabs')
      .select('total_iqd, court_iqd, settled_at')
      .eq('id', tabId)
      .single();
  ...
  it('ops_overview carries every contracted key and the seeded tab shows on the open day', async () => {
    ...
    const o = res.data as Record<string, Record<string, unknown>> & {
    ...
    for (const k of ['discounts', 'refunds', 'voids'] as const) {
      expect(Object.keys((o.exceptions as Record<string, object>)[k]!).sort()).toEqual([
        'amountIqd',
        'count',
      ]);
    }
    expect(Object.keys((o.exceptions as Record<string, object>).waste!).sort()).toEqual([
      'costIqd',
      'count',
    ]);
  ...
  it('panel_headline: revenue = padel + cafe, both include the fixture, cash reconciles with payments - refunds', async () => {
    const res = await appRpc(owner, 'panel_headline', {
      p_from: from,
      p_to: to,
      p_compare: 'none',
    }).then(outcome);
    ...
    expect(byKey.avgOrderValue!.value).toBe(
      Math.round(byKey.cafeRevenue!.value / byKey.orders!.value),
    );
    ...
    const paid = (m: string) =>
      payRows.filter((p) => p.method === m).reduce((s, p) => s + Number(p.amount_iqd), 0);
    const refunded = (m: string) =>
      refRows
        .filter((r) => payRows.find((p) => p.id === r.payment_id)?.method === m)
  ...
  it('panel_headline comparison: previousPeriod / sameLastYear windows, changeAbs arithmetic, changePct null on zero', async () => {
    ...
    const prev = await appRpc(owner, 'panel_headline', {
      p_from: from,
      p_to: to,
      p_compare: 'previousPeriod',
    }).then(outcome);
    ...
    for (const f of p.figures) {
      ...
      if ((f.previous as number) === 0) expect(f.changePct, f.key).toBeNull();
        ...
        expect(Number(f.changePct)).toBeCloseTo(
          ((f.value - (f.previous as number)) * 100) / (f.previous as number),
          0,
        );
    }

    const ly = await appRpc(owner, 'panel_headline', {
      p_from: from,
      p_to: to,
      p_compare: 'sameLastYear',
    }).then(outcome);
    ...
    const bad = await appRpc(owner, 'panel_headline', {
      p_from: from,
      p_to: to,
      p_compare: 'lastWeek',
    }).then(outcome);
    expect(bad.errorMessage).toContain('INVALID_ARGUMENT');
    const inverted = await appRpc(owner, 'panel_headline', {
      p_from: to,
      p_to: from,
      p_compare: 'none',
    }).then(outcome);
  ...
  it('report_revenue groups by day; totals equal the sum of the rows; the fixture lands on its days', async () => {
    const res = await appRpc(owner, 'report_revenue', {
      p_from: from,
      p_to: to,
      p_group: 'day',
      p_filters: {},
    ...
    const numeric = d.columns
      .map((c) => c.key)
      .filter((k) => k !== 'period') as (keyof RevenueRow)[];
  ...
  it('report_revenue: week / month buckets keep the same totals; filters narrow; bad arguments refused', async () => {
    const day = await appRpc(owner, 'report_revenue', {
      p_from: from,
      p_to: to,
      p_group: 'day',
      p_filters: {},
    }).then(outcome);
    const week = await appRpc(owner, 'report_revenue', {
      p_from: from,
      p_to: to,
      p_group: 'week',
      p_filters: {},
    }).then(outcome);
    const month = await appRpc(owner, 'report_revenue', {
      p_from: from,
      p_to: to,
      p_group: 'month',
      p_filters: {},
    ...
    const byCashier = await appRpc(owner, 'report_revenue', {
      ...
      p_to: to,
      p_group: 'day',
      p_filters: { staffId: SEED_STAFF_IDS.cashier },
    }).then(outcome);
    ...
    const badGroup = await appRpc(owner, 'report_revenue', {
      p_from: from,
      p_to: to,
      p_group: 'year',
      p_filters: {},
  ...
  it('report_courts: the fixture court shows its booking, available minutes come from opening hours, byHour has 24 buckets', async () => {
    const res = await appRpc(manager, 'report_courts', {
      p_from: from,
      p_to: to,
      p_filters: { courtId },
    }).then(outcome);
    ...
    const d = res.data as {
      ...
      trend: { date: string; bookings: number; revenueIqd: number }[];
    };
  ...
  it('report_cafe: the settled item is a row with qty, revenue and null (never 0) cogs when it has no recipe', async () => {
    const res = await appRpc(manager, 'report_cafe', {
      p_from: from,
      p_to: to,
      p_filters: { categoryId: item.categoryId },
    }).then(outcome);
    expect(res.ok, res.errorMessage).toBe(true);
    type CafeRow = {
      itemId: string;
  ...
  it('report_stock: value on hand, the five lists and the ranged sections are present', async () => {
    const res = await appRpc(manager, 'report_stock', {
      p_from: from,
      p_to: to,
      p_filters: {},
    }).then(outcome);
  ...
  it('report_staff_activity: rows ordered by name, no rank/score field, the fixture actions attributed', async () => {
    const res = await appRpc(manager, 'report_staff_activity', {
      p_from: from,
      p_to: to,
      p_staff_id: null,
    }).then(outcome);
    ...
    type StaffRow = {
      ...
      discounts: { count: number; amountIqd: number };
      voids: { count: number; amountIqd: number };
      refunds: { count: number; amountIqd: number };
      waiterCallResponse: { count: number; avgSeconds: number | null };
      dayCloses: { businessDate: string; cashVarianceIqd: number }[];
      shiftContext: { daysWorked: number; busiestDayOrders: number };
    };
    ...
    const one = await appRpc(manager, 'report_staff_activity', {
      p_from: from,
      p_to: to,
      p_staff_id: SEED_STAFF_IDS.cashier,
    }).then(outcome);
    expect((one.data as { rows: StaffRow[] }).rows.map((r) => r.staffId)).toEqual([
  ...
  it('report_drill: cafeRevenue returns the settled tab; padelRevenue the booking; scopes narrow; owner-only for money', async () => {
    const cafe = await appRpc(owner, 'report_drill', {
      p_figure: 'cafeRevenue',
      p_key: null,
      p_from: from,
      p_to: to,
    }).then(outcome);
    ...
    const padel = await appRpc(owner, 'report_drill', {
      p_figure: 'padelRevenue',
      p_key: `court:${courtId}`,
      p_from: from,
      p_to: to,
    }).then(outcome);
    ...
    const cash = await appRpc(owner, 'report_drill', {
      p_figure: 'cash',
      p_key: `staff:${SEED_STAFF_IDS.cashier}`,
      p_from: from,
      p_to: to,
    }).then(outcome);
    ...
    expect(
      ctx.some(
        (t) => t.kind === 'payment' && t.reference === tabId && Number(t.amountIqd) === tabTotal,
      ),
    ).toBe(true);

    const byItem = await appRpc(manager, 'report_drill', {
      p_figure: `item:${item.itemId}`,
      p_key: null,
      p_from: from,
      p_to: to,
    }).then(outcome);
    ...
    const byCourt = await appRpc(manager, 'report_drill', {
      p_figure: `court:${courtId}`,
      p_key: null,
      p_from: from,
      p_to: to,
    }).then(outcome);
    expect((byCourt.data as { transactions: Drill[] }).transactions.map((t) => t.id)).toEqual([
      reservationId,
    ]);

    for (const fig of ['revenue', 'padelRevenue', 'cafeRevenue', 'cash', 'card']) {
      const m = await appRpc(manager, 'report_drill', {
        p_figure: fig,
        p_key: null,
        p_from: from,
        p_to: to,
    ...
    const bad = await appRpc(owner, 'report_drill', {
      p_figure: 'profit',
      p_key: null,
      p_from: from,
      p_to: to,
    ...
    expect(bad.errorMessage).toContain('INVALID_ARGUMENT');
    const badKey = await appRpc(owner, 'report_drill', {
      p_figure: 'bookings',
      p_key: 'table:1',
      p_from: from,
      p_to: to,
  ...
  it('audit_log_page: filters by actor and action prefix server-side, names the actor, pages with a total', async () => {
    ...
    expect(
      (byActor.data as { rows: AuditRow[] }).rows.every(
        (r) => r.actorName === 'Dev Cashier' || r.authorizerName === 'Dev Cashier',
      ),
    ).toBe(true);

    const page1 = await appRpc(manager, 'audit_log_page', {
      p_from: null,
      p_to: null,
      p_limit: 1,
      p_offset: 0,
    }).then(outcome);
    const page2 = await appRpc(manager, 'audit_log_page', {
      p_from: null,
      p_to: null,
      p_limit: 1,
      p_offset: 1,
    ...
    const bad = await appRpc(manager, 'audit_log_page', {
      p_from: null,
      p_to: null,
      p_limit: 0,
      p_offset: 0,
