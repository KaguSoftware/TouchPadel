...
describe.skipIf(!up)(
  'cafe menu extensions (0027-0031: sold_out / photo / cost / reveals / settings / bell / storage)',
  () => {
    ...
    it('menu_item_costs: manager reads it, cashier / guest get RLS silence, anon is denied', async () => {
      ...
      const neg = await appRpc(manager, 'set_item_cost', {
        p_item_id: plain.itemId,
        p_cost_iqd: -1,
      }).then(outcome);
      expect(neg.errorMessage).toContain('INVALID_COST');
      const clear = await appRpc(manager, 'set_item_cost', {
        p_item_id: plain.itemId,
        p_cost_iqd: null,
      }).then(outcome);
      ...
      const noDrink = await order(guest.client, [
        {
          variant_id: meal.variantId,
          qty: 1,
          modifiers: [{ modifier_id: mealModifierId, qty: 1 }],
        },
      ]);
    ...
    it('set_modifier_reveals: REVEAL_SELF / REVEAL_DEPTH (both directions) / manager ok / guest FORBIDDEN', async () => {
      ...
      await addRevealGroup(svc, g3.modifierId, { min: 0, max: 1 }, [
        { nameAr: 'فرعي', deltaIqd: 0 },
      ]);
    ...
    it('featured 15 % promo: unit = applyPctDiscountIqd(list, 15), list kept, discount_source=featured', async () => {
      ...
      const { data: lines } = await svc
        .from('order_items')
        .select(
          'menu_item_id, unit_price_iqd, list_price_iqd, discount_pct, discount_source, line_total_iqd',
        )
        .eq('order_id', orderId);
      type Line = {
        menu_item_id: string;
    ...
    it('app.apply_pct_discount parity with @touch/core applyPctDiscountIqd (pct 0..99 x list prices)', async () => {
      ...
      for (let i = 0; i < cases.length; i += BATCH) {
        ...
        results.forEach((r, j) => {
          ...
          expect(Number(r.data), `${c.list} @ ${c.pct}%`).toBe(
            Number(applyPctDiscountIqd(c.list, c.pct)),
          );
        });
      }
    ...
    it('settings split: anon + guest read cafe_settings_public without any telegram_/analytics_ key; base table is staff-only', async () => {
      for (const [label, client] of [
        ...
        expect(
          keys.some((k) => k.startsWith('telegram_')),
          label,
        ).toBe(false);
        expect(
          keys.some((k) => k.startsWith('analytics_')),
          label,
        ).toBe(false);
    ...
    it('set_cafe_setting: manager FORBIDDEN on owner keys, owner ok; UNKNOWN_SETTING; INVALID_SETTING_VALUE cases', async () => {
      ...
      const g = await appRpc(guest.client, 'set_cafe_setting', {
        p_key: 'hero_mode',
        p_value: 'none',
      }).then(outcome);
      expect(g.errorMessage).toContain('FORBIDDEN');
      const c = await appRpc(cashier, 'set_cafe_setting', {
        p_key: 'hero_mode',
        p_value: 'none',
      }).then(outcome);
      expect(c.errorMessage).toContain('FORBIDDEN');

      const unknown = await appRpc(owner, 'set_cafe_setting', { p_key: 'nope', p_value: 1 }).then(
        outcome,
      );
      ...
      for (const [key, value] of invalid) {
        const r = await appRpc(owner, 'set_cafe_setting', { p_key: key, p_value: value }).then(
          outcome,
        );
        expect(r.ok, `${key}=${JSON.stringify(value)}`).toBe(false);
        expect(r.errorMessage, `${key}=${JSON.stringify(value)}`).toContain(
      ...
      const nul = await appRpc(owner, 'set_cafe_setting', {
        p_key: 'telegram_chat_id',
        p_value: null,
      }).then(outcome);
    ...
    it('bell: set_table_bell(false) -> guest raise_waiter_call BELL_DISABLED; open_table_session returns bell_enabled', async () => {
      const off = await appRpc(owner, 'set_table_bell', {
        p_table_id: tableId,
        p_enabled: false,
      }).then(outcome);
      expect(off.ok, off.errorMessage).toBe(true);

      const refused = await appRpc(guest.client, 'raise_waiter_call', { p_reason: 'water' }).then(
        outcome,
      );
      expect(refused.ok).toBe(false);
      ...
      const opened = await appRpc(scanner, 'open_table_session', { p_token: tok.data }).then(
        outcome,
      );
      expect(opened.ok, opened.errorMessage).toBe(true);
      const o = opened.data as {
        session_id: string;
        table_id: string;
        table_number: string;
      ...
      const on = await appRpc(manager, 'set_table_bell', {
        p_table_id: tableId,
        p_enabled: true,
      }).then(outcome);
      expect(on.ok, on.errorMessage).toBe(true);
      const reopened = await appRpc(scanner, 'open_table_session', { p_token: tok.data }).then(
        outcome,
      );
      expect((reopened.data as { bell_enabled: boolean }).bell_enabled).toBe(true);

      const guestTry = await appRpc(guest.client, 'set_table_bell', {
        p_table_id: tableId,
        p_enabled: false,
      }).then(outcome);
      expect(guestTry.errorMessage).toContain('FORBIDDEN');
      const cashierTry = await appRpc(cashier, 'set_table_bell', {
        p_table_id: tableId,
        p_enabled: false,
      }).then(outcome);
      expect(cashierTry.errorMessage).toContain('FORBIDDEN');
    ...
    it('table_qr_tokens: every active table with a token that open_table_session accepts; upsert_cafe_table CRUD', async () => {
      ...
      const scanner = await anonymousSessionClient();
      const opened = await appRpc(scanner, 'open_table_session', { p_token: mine!.token }).then(
        outcome,
      );
      expect(opened.ok, opened.errorMessage).toBe(true);
      ...
      const { data: t } = await svc
        .from('cafe_tables')
        .select('table_number, capacity, bell_enabled')
        .eq('id', newId)
        .single();
      expect(t).toEqual({ table_number: `${num}-B`, capacity: 8, bell_enabled: true });

      const taken = await appRpc(manager, 'upsert_cafe_table', { p_table_number: `${num}-B` }).then(
        outcome,
      );
      expect(taken.errorMessage).toContain('TABLE_NUMBER_TAKEN');
      const blank = await appRpc(manager, 'upsert_cafe_table', { p_table_number: '  ' }).then(
        outcome,
      );
    ...
    it('storage menu-media: manager uploads items/x/y.webp, guest upload rejected, anon download ok', async (ctx) => {
      const bucket = await svc.storage
        .getBucket('menu-media')
        .catch(() => ({ data: null, error: { message: 'unreachable' } }));
      ...
      const guestUpload = await guest.client.storage
        .from('menu-media')
        .upload(`items/${plain.itemId}/guest-${Date.now()}.webp`, bytes, {
      ...
      const cashierUpload = await cashier.storage
        .from('menu-media')
        .upload(`items/${plain.itemId}/cashier-${Date.now()}.webp`, bytes, {
          contentType: 'image/webp',
        });
